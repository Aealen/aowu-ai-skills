...
### 规则 17.5：PyMuPDF 表格 colspan/rowspan 推断——边界点索引差值，非计数

`_infer_colspan_rowspan` 从 PyMuPDF Table 的 cell 物理边界推断合并单元格。
两个关键修复（否则简单表格列数据大量丢失）：

**修复1：colspan/rowspan 用边界点索引差值，非"覆盖了多少个边界点"**

```
❌ 旧（错误）：遍历 col_xs，统计 cell 的 x 范围覆盖了多少个边界点
   cell0 bbox=(90,136)，col_xs=[90,136,182,...]
   90 满足、136 也满足 → colspan=2 ❌（每个正常cell都跨了2个边界点：自己的左右端）

✓ 新（正确）：cell 左边界对齐 col_xs[i]，右边界对齐 col_xs[j]，colspan = j - i
   cell0 bbox=(90,136)，col_xs 中 90→索引0，136→索引1 → colspan=1-0=1 ✓
```

**修复2：列边界遍历所有行收集，非只看第一行**

第一行可能是跨满宽的合并 cell（colspan=N，如"对本项目商务条款的偏离情况..."），
只有1个 cell，列边界只有2个点（左/右）→ n_cols 严重低估为1。
遍历所有行的 cell x 坐标取并集，确保收集到所有列分隔线。
并用 `pymupdf_table.col_count` 交叉验证。

**修复3：_fill_table_cells 跳过 rowspan=0/colspan=0 的占位 cell**

`_infer_colspan_rowspan` 对被合并位置返回 `{"rowspan":0, "colspan":0}`。
`_fill_table_cells` 遍历 rows_data 时必须跳过这些占位（c_idx+=1, continue），
否则会在合并后的格子上重复写入。

**数据验证**：6个表格（费用计算表/报价一览表/人员汇总表/项目情况表/偏离表等）
从列重复丢失恢复为完整列：
| 表格 | 修复前 | 修复后 |
|------|-------|-------|
| 费用计算表 | 5列(重复) | **9列**(序号/项目名称/规模/工日定额/...) |
| 报价一览表 | 4列(重复) | **4列**(序号/供应商名称/响应报价/合同期限) |
| 人员汇总表 | 5列(重复) | **5列**(序号/姓名/职称/岗位名称/...) |
...
**注意**：
- align.py 在 build_docx.py 之前运行，读的是原始未拆分数据，跨页匹配逻辑（规则20）不受影响
- 拆分后续行 line 携带的 `_style`（align 阶段附加）跟着 line 对象走，无需额外处理
- 只拆 `text`/`paragraph`/`list` 类型，`table` 跨页用独立的 `_table_merged` 机制（规则9）

### 规则 19：非标题页的强分页——上页正文未填满 + 本页页首

PDF 中有些页面的首个 block 不是章节标题（`type≠title`），但 PDF 有强制分页意图——
上页正文只写了一部分就翻页了。MinerU 不反映这种分页意图，需从坐标特征推断。

**典型场景**："响应文件封面（参考格式）"——第七章内的子标题参考格式页。
MinerU 标为 `type=text`（非 title），PDF 中上页（"供应商编制文件须知"末尾）只写到
y1=280 就翻页了（正常页 y1≈700-760），本页 block#0 在页首（y0=93）。

**判据**（主循环分页逻辑，四个条件全部满足）：
1. 本页 block#0 是 text/paragraph（非 title，非 table，非跨页续行）
2. 本页 block#0 在页首（y0 < 页高 × 14%）
3. 上一页最后一个有内容的 block 也是 text/paragraph（排除表格续页干扰）
4. 上页最后 block 的 y1 明显低于正常页底（< 页高 × 60%）

**为何条件3+4缺一不可**：
- 单独用条件4（y1低）会误判表格页——表格 bbox 不含完整内容区，prev_last_y1 天然偏低
  （如 pi=9/49/53/54/55/56 表格页 y1=0~501，但它们不需要分页）
- 加条件3（上页最后block是text）排除表格干扰——表格页的最后block是table不是text
- 全文档仅 pi=62 满足全部四条件，无误伤

**验证**：修复后总页数 88 页 = 原始 PDF 88 页，逐页对齐；"响应文件封面"在第63页页首
（=原始PDF第63页）。

### 规则 20：跨页 block 续行 span 样式——邻页 spans 匹配 + 主导样式兜底
...
| 跨页block用全部lines数算行高 | bbox只覆盖本页(15pt)但lines含续页行(5行)→3pt行距，文字被EXACTLY压扁挤一行（1.5.2/可研正文8处）| 拆分跨页block为本页+续页两部分，各自bbox高/行数(规则18) |
| 跨页block用续行y0差值做行高 | 行距准(24.7pt)但5行总高123pt远超bbox15pt→内容连锁后移，1.5.2推到下页 | 拆分后各部分独立计算，总高=PDF(规则18) |
...
| 跨页block续行用本页spans匹配 | 续行坐标在下一页，本页IoU=0匹配不上→无样式(继承Word默认)；或containment误匹配→字体串扰(SimHei15.9pt) | 检测跨页后合并邻页spans匹配 + 无样式span用主导样式兜底(规则20) |
| 分页只认type=title的首个block | 非标题的强分页页(如"响应文件封面"参考格式页,type=text)不触发分页→紧接上文,DOCX比PDF少1页 | 增加判据：text页首+上页text未填满(y1<60%页高)也插分页符(规则19) |
| 用prev_last_y1<阈值单独判强分页 | 表格页bbox不含完整内容区,y1天然偏低(0~501)→表格续页被误插分页符 | 加条件：上页最后block须是text(排除表格) (规则19) |
